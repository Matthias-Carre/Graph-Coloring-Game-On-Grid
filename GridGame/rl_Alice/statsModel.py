"""
Stats script: run Alice's CNN model against Bob (random or heuristic) over many games
and report win rates, similar to rl_Bob/statsModel.py.

Usage:
    python statsModel.py --w 6 --h 6 --colors 4 --games 500 --bob random
    python statsModel.py --w 6 --h 6 --colors 4 --games 500 --bob heuristic
    python statsModel.py --w 6 --h 6 --colors 4 --games 500 --bob all
"""
...
def run_model_vs_bob(width, height, num_colors, num_games, bob_mode, model, device):
    """
    Alice = CNN model, Bob = random or heuristic.
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Alice's CNN model win-rate vs Bob.")
    ...
    for mode in bob_modes:
        stats = run_model_vs_bob(args.w, args.h, args.colors, args.games, mode, model, device)
