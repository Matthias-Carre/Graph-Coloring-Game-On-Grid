...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract and display feature maps from a GraphColoringNet model.")
    ...
    # 2. Loading the brain using the provided argument path
    ...
    try:
        ...
        model.eval()
        print(f"Model loaded successfully from {MODEL_PATH}. Extracting Feature Maps...")
    ...
    # Fix the UserWarning from Matplotlib regarding add_axes and tight_layout
    plt.subplots_adjust(left=0.05, right=0.90, top=0.90, bottom=0.05, wspace=0.1, hspace=0.3)
    plt.show()
